Pfam.gene.num <- function(pfams, title) {
  #Creates a function where the Pfam and title of output file needs to be input
  ...
  for (i in 1:num.species) {genes.temp <- getBM(mart = mart, 
                                                filters = c("species_id_1010", "pfam_pf"),
                                                value = list(all.species.ids$species_id[i], pfams),
                                                attributes = "wbps_gene_id")
  ##get all genes containing the Pfam IDs in species
